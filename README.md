# 📡 Tracking Stack — rastreamento server-side na SUA conta

> Seu próprio rastreamento de Meta Ads, GA4 e Google Ads rodando no **seu** Cloudflare —
> substitui Stape/GTM Server-Side, sem mensalidade. Cookies que sobrevivem ao Safari (ITP),
> eventos server-side (CAPI), atribuição completa e um dashboard pronto.

## 🚀 Como começar (3 passos)

1. **Gere a sua cópia** — botão verde **`Use this template`** no topo desta página → `Create a new repository`.
2. **Baixe na sua máquina** — `git clone` do seu novo repo + `cd` na pasta. (Windows? veja [`docs/windows.md`](docs/windows.md).)
3. **Abra o Claude Code e diga `instalar`** — rode `claude` na pasta e escreva **`instalar`**.
   O Claude conduz tudo (login Cloudflare, banco D1, GitHub, variáveis). Acompanhe em [`aula/roteiro.md`](aula/roteiro.md).

> A referência técnica completa (em inglês) está logo abaixo e em `docs/`. Você não
> precisa lê-la pra instalar — o Claude faz isso por você.

---

## Tracking Stack (technical reference)
