...
> **Aluno fala português.** Gatilhos PT equivalem aos EN abaixo: "instalar",
> "começar", "configurar", "acabei de baixar", "setup" → `deploy-stack`.
> "tá funcionando?", "não aparece conversão" → `verify-tracking`. "adicionar
> página"/"nova landing" → `add-page`. Responda sempre em português, simples.
